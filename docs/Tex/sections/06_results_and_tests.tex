...
\begin{itemize}
    \item prestazioni: tempo massimo di risposta (latenza HTTP, escluso il tempo di attuazione fisica) $< 1$ s (NFR2);
...
La fase di testing ha previsto sia simulazioni controllate — incluse prove di osservazione controllata del comportamento del modulo GPS \cite{He2019} — sia test in condizioni reali, con valutazioni sull'effetto della latenza di rete, dei disturbi radio, dei ritardi nella sincronizzazione GPS, della congestione del broker MQTT e dell'uso contemporaneo da parte di più utenti (FR6).
...
\begin{table}[H]
\centering
\caption{Tracciabilità dei requisiti funzionali (FR) e non funzionali (NFR) rispetto ai capitoli progettuali e ai test di validazione}
\label{tab:fr_nfr_mapping}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|c|p{4.2cm}|c|p{5.2cm}|}
\hline
\textbf{ID} & \textbf{Descrizione requisito} & \textbf{Capitolo} & \textbf{Test di validazione} \\
\hline
FR1 & Apertura remota del portone garage & Cap. 4–5 & Test funzionale MQTT/HTTP, verifica comando attuatore \\
\hline
FR2 & Chiusura remota controllata & Cap. 4–5 & Test di chiusura manuale e remota \\
\hline
FR3 & Notifica stato porta all’utente & Cap. 5 & Test Telegram Bot (event-driven) \\
\hline
FR4 & Chiusura automatica temporizzata & Cap. 5 & Test temporale su Arduino (timeout) \\
\hline
FR5a & Automazione in uscita (PIR) & Cap. 4–5 & Test sensore PIR e trigger locale \\
\hline
FR5b & Automazione in ingresso (GPS) & Cap. 4–5 & Test geofence e ricezione coordinate MQTT \\
\hline
FR6 & Gestione multiutenza & Cap. 4–5 & Test accesso concorrente e gestione utenti \\
\hline
FR7 & Comando manuale locale & Cap. 5 & Test pulsante fisico \\
\hline
FR8 & Rilevazione ostacoli (safety) & Cap. 5 & Test sensore ultrasonico \\
\hline
FR9 & Funzionamento offline & Cap. 4–5 & Test senza connettività Internet \\
\hline
NFR1 & Bassa latenza end-to-end & Cap. 6 & Misura tempi di risposta MQTT e HTTP \\
\hline
NFR2 & Affidabilità operativa & Cap. 6 & Test ripetuti su più cicli di apertura/chiusura \\
\hline
NFR3 & Scalabilità architetturale & Cap. 4 & Analisi architettura modulare a livelli IoT \\
\hline
NFR4 & Sicurezza delle comunicazioni & Cap. 4–5 & Test autenticazione utenti e isolamento dei canali \\
\hline
NFR5 & Robustezza a fault locali & Cap. 6 & Test guasti sensori e meccanismi di recovery \\
\hline
NFR6 & Manutenibilità & Cap. 5 & Analisi modularità firmware e separazione dei servizi \\
\hline
NFR7 & Disponibilità del servizio & Cap. 6 & Monitoraggio periodico dello stato tramite processo timer \\
\hline
NFR8 & Osservabilità del sistema & Cap. 6 & Logging eventi, stato e metriche di performance \\
\hline
NFR9 & Riproducibilità dell’ambiente & Cap. 5 & Verifica installazione deterministica tramite \texttt{requirements.txt} \\
\hline
NFR10 & Usabilità dell’interfaccia utente & Cap. 5–6 & Test interazione utente via Telegram Bot \\
\hline
\end{tabular}
\end{table}


La Tabella~\ref{tab:fr_nfr_mapping} fornisce una vista di tracciabilità completa tra i requisiti definiti in fase di analisi, le scelte progettuali e le attività di test svolte, dimostrando la coerenza dell’intero processo di sviluppo secondo un approccio System Development Life Cycle (SDLC).
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item modulo GPS (accuratezza del geofence, stabilità della distanza calcolata);
...
\subsection{Mappatura tra metodologia di test e strumenti software utilizzati}
Al fine di garantire una validazione rigorosa, riproducibile e metodologicamente difendibile del sistema Smart Garage Door, ciascuna fase di testing descritta nella sezione precedente è stata associata in modo esplicito a specifici strumenti software e script di test sviluppati ad hoc nell’ambito del progetto. Tale mappatura consente di collegare in maniera chiara i risultati sperimentali presentati nelle sezioni successive ai file effettivamente utilizzati per l’esecuzione delle prove, evitando ambiguità tra test concettuali e implementazioni operative.

\paragraph{Unit e Integration Testing del livello applicativo}
Il unit e integration testing del backend applicativo è stato condotto tramite lo script \texttt{test\_app.py}, sviluppato utilizzando il framework \textit{pytest}. Questo file consente di testare in isolamento il server Flask, verificando il corretto funzionamento degli endpoint REST, la gestione dello stato della porta, l’elaborazione dei comandi utente e i meccanismi di autenticazione (NFR6).
Durante questa fase, la comunicazione MQTT è stata deliberatamente disabilitata o simulata, al fine di isolare la logica applicativa ed escludere effetti dovuti alla rete o ai dispositivi embedded. Questo approccio è coerente con le buone pratiche di unit testing per sistemi distribuiti, che raccomandano l’isolamento dei componenti software per garantire determinismo e ripetibilità dei test.

\paragraph{Integration e System Testing della pipeline di comunicazione}
La verifica dell’interoperabilità tra i diversi livelli dell’architettura IoT (Network e Application Layer) è stata eseguita tramite lo script \texttt{mqtt\_test.py}. Tale strumento simula il comportamento del nodo NodeMCU ESP8266, pubblicando e sottoscrivendo messaggi MQTT realistici sui topic del sistema (\texttt{home/garage/\#}).
Questo script è stato utilizzato per validare la corretta propagazione degli eventi lungo la catena MQTT $\rightarrow$ Flask $\rightarrow$ Telegram, nonché per testare scenari di riconnessione, perdita temporanea del broker e stabilità del flusso dati. In questo contesto, \texttt{mqtt\_test.py} rappresenta l’elemento di raccordo tra integration testing e system testing, permettendo una valutazione realistica del comportamento distribuito del sistema.

\paragraph{Test prestazionali e misure temporali}
Le misure di latenza e di prestazioni temporali presentate nella sezione dei risultati sono state ottenute mediante script dedicati di monitoraggio (\texttt{performance\_monitor.py} e \texttt{timer.py}). Tali strumenti sono stati utilizzati per:
\begin{itemize}
    \item misurare la latenza HTTP di request/response tra il comando dell’utente e il backend applicativo, in accordo con il requisito NFR2;
    \item calcolare statistiche descrittive (media e deviazione standard) su più iterazioni sperimentali;
    \item distinguere esplicitamente tra latenza del sistema di controllo software e tempo complessivo end-to-end, comprensivo dell’attuazione elettromeccanica del servo.
\end{itemize}
Questa separazione metodologica consente di attribuire correttamente le componenti temporali osservate alle rispettive responsabilità architetturali, evitando di includere nel requisito NFR2 elementi deliberatamente progettati per garantire sicurezza e affidabilità meccanica.

Nel complesso, l’utilizzo combinato di \texttt{test\_app.py}, \texttt{mqtt\_test.py} e degli script di monitoraggio temporale garantisce una copertura completa delle fasi di unit, integration e system testing. La mappatura esplicita tra metodologia di test, strumenti software e risultati sperimentali rafforza la solidità scientifica della validazione e rende il processo pienamente tracciabile e riproducibile.
\begin{table}[h!]
\centering
\caption{Mappatura tra tipologie di test, strumenti utilizzati e requisiti verificati}
\label{tab:test_file_mapping}
\begin{tabular}{|p{3.6cm}|p{4.4cm}|p{6.0cm}|}
\hline
\textbf{Tipologia di test} & \textbf{File / Strumento utilizzato} & \textbf{Requisiti verificati} \\ \hline

Unit Testing (Application Layer) &
\texttt{test\_app.py} &
FR1, FR2, FR3, FR6, FR9; NFR6, NFR7 \\ \hline

Integration Testing (MQTT--HTTP) &
\texttt{mqtt\_test.py} &
FR1--FR3, FR5b, FR6; NFR1, NFR2, NFR8 \\ \hline

Test automazione in uscita (PIR) &
Sistema reale (Arduino + sensore PIR) &
FR5a; NFR5 \\ \hline

Test accuratezza geofence (GPS) &
Firmware NodeMCU + modulo GPS reale &
FR5b; NFR3, NFR4 \\ \hline

System Testing (end-to-end) &
Sistema reale (Arduino + NodeMCU + Flask + Telegram) &
FR1--FR9; NFR1--NFR5 \\ \hline

Test prestazionali (latenza HTTP) &
\texttt{performance\_monitor.py}, \texttt{timer.py} &
NFR2 \\ \hline

Test tempo complessivo end-to-end &
\texttt{performance\_monitor.py} + attuatore reale &
Analisi progettuale (escluso NFR2) \\ \hline

Test funzionamento offline &
Test manuale su hardware reale &
FR4, FR7; NFR5 \\ \hline

Test robustezza e fault tolerance &
\texttt{mqtt\_test.py} + test di resilienza con degradazione controllata &
NFR1, NFR5, NFR8 \\ \hline

Test efficienza energetica &
Misure sperimentali su ESP8266 e GPS &
NFR9 \\ \hline

Verifica costo complessivo &
Analisi distinta dei componenti hardware &
NFR10 \\ \hline
...
\begin{itemize}
    \item \textbf{Tempo medio di risposta}: latenza HTTP tra l’invio del comando dell’utente e la ricezione della risposta dal sistema applicativo, escludendo il tempo di attuazione fisica ($< 1$ s, NFR2).
...
\begin{table}[h!]
    ...
    FR5b & Apertura automatica in ingresso quando l'utente entra nel geofence GPS (raggio 15 m); verifica transizioni 0$\rightarrow$1 e 1$\rightarrow$0 nel topic \texttt{home/garage/gps}. & Superato \\ \hline
...
\begin{table}[h!]
    ...
    FR5a - Automazione in uscita & Simulazione di movimento rilevato dal PIR con porta chiusa; verifica dell'attivazione immediata del relè; misurazione del tasso di falsi positivi in condizioni di luce variabile; test in condizioni ambientali variabili (luce e temperatura). & Superato \\ \hline
    FR5a - Robustezza & Introduzione di rumore artificiale sul pin PIR per simulare malfunzionamenti; verifica della robustezza della logica contestuale basata su stato porta e prossimità GPS, e assenza di aperture spurie. & Superato \\ \hline
    FR5b - Automazione in ingresso (GPS) & Test del geofence a 15 m con transizioni multiple "fuori $\rightarrow$ dentro $\rightarrow$ fuori"; verifica della pubblicazione stabile sul topic MQTT \texttt{home/garage/gps}. & Superato \\ \hline
    FR5b - Stabilità GPS &  Valutazione della stabilità del geofence in presenza di aggiornamenti di posizione irregolari; verifica del comportamento stateful del sistema e assenza di attivazioni spurie durante le transizioni di ingresso/uscita. &  Superato \\ \hline
...
La metodologia adottata ha incluso misure sperimentali ripetute, prove di stress e test di carico e scenari di fault injection controllato. Sono state inoltre condotte valutazioni di energy profiling e misure di latenza complessiva del sistema, distinguendo tra latenza del sistema di controllo e tempo di attuazione fisica, così come raccomandato nelle linee guida per sistemi distribuiti real-time \cite{Lee2015} e architetture IoT ad alta disponibilità \cite{Zanella2014}.
...
\begin{table}[h!]
    ...
    NFR2 & Misura della latenza HTTP request/response tra comando utente e backend applicativo (esclusa attuazione fisica): 0.211 s (OPEN), 0.225 s (CLOSE), n=10. & OK \\ \hline
    NFR3 & Accuratezza del geofence: errore medio inferiore a 3--5 m e tasso di falsi positivi $<$ 1\% & OK \\ \hline
    NFR4 & Stabilità della rilevazione entro un raggio effettivo di 17 m, coerente con soglia progettuale di 15 m. & OK \\ \hline
    ...
    NFR6 & Sicurezza applicativa: autenticazione HTTP con username e password, protezione delle comunicazioni Telegram tramite MTProto \cite{Kuznetsov2018}. & OK \\ \hline
    ...
    NFR9 & Consumo energetico: 0.42 W (idle), 0.55 W (attività), conforme alle linee guida per nodi IoT low-power \cite{Gubbi2013}. & OK \\ \hline
    NFR10 & Costo complessivo del prototipo pari a 92.30 €, al di sotto del limite di progetto di 150 €. & OK \\ \hline
    \end{tabular}
\end{table}

I risultati confermano la piena conformità agli obiettivi prestazionali del progetto. La pipeline di comunicazione UART, Wi-Fi, MQTT e HTTP REST ha mantenuto una latenza del sistema di controllo inferiore al secondo anche in condizioni di carico elevato, dimostrando una notevole reattività dell’architettura applicativa. L'accuratezza del modulo GPS e l'efficienza del meccanismo di geofence soddisfano pienamente i requisiti relativi all'automazione di prossimità (FR5a-FR5b) e confermano l'efficacia della strategia event-driven adottata \cite{Sanchez2018}. Il consumo energetico complessivo è risultato compatibile con scenari di alimentazione a batteria (NFR9), mentre il costo ridotto dei componenti conferma la sostenibilità economica dell'implementazione (NFR10), in linea con quanto previsto nelle fasi di planning e design.


\subsection{Prestazioni e valutazione temporale}

Per valutare la reattività e l’efficienza del sistema \textit{Smart Garage Door}, sono stati misurati i principali indicatori prestazionali lungo l’intera catena di controllo, distinguendo tra latenza di comunicazione e tempo complessivo di risposta end-to-end. In particolare, le misure considerano sia la latenza introdotta dallo stack di rete e dal livello applicativo (HTTP/REST, MQTT, Flask), sia il contributo dovuto all’attuazione meccanica del sistema (movimento del servo).

I risultati sperimentali mostrano che la latenza di comunicazione HTTP pura (richiesta/risposta) si mantiene mediamente inferiore a 250~ms, garantendo un’elevata reattività dell’interfaccia utente e piena conformità ai requisiti non funzionali di tempestività e affidabilità della comunicazione (NFR2, NFR3). Il tempo totale percepito dall’utente, comprensivo dell’attuazione fisica della porta, risulta invece dominato dal movimento meccanico del servo, attestandosi intorno a 1.6–1.7~s.

\begin{table}[h!]
    \centering
    \caption{Prestazioni temporali del sistema \textit{Smart Garage Door}: latenza di comunicazione e tempo end-to-end.}
    \label{tab:performance}
    \begin{tabular}{|p{6.5cm}|c|c|}
    \hline
    \textbf{Operazione} & \textbf{Tempo medio (s)} & \textbf{Dev. std (s)} \\ \hline
    Latenza HTTP -- comando OPEN (request/response) & 0.211 & 0.081 \\ \hline
    Latenza HTTP -- comando CLOSE (request/response) & 0.225 & 0.067 \\ \hline
    Tempo totale OPEN (HTTP + attuazione servo) & 1.675 & 0.068 \\ \hline
    Tempo totale CLOSE (HTTP + attuazione servo) & 1.631 & 0.056 \\ \hline
    \end{tabular}
\end{table}

La separazione tra latenza di comunicazione e tempo di risposta complessivo consente di evidenziare come l’infrastruttura software e di rete introduca un overhead minimo, mentre la componente predominante del ritardo percepito sia riconducibile all’attuazione elettromeccanica. L’adozione di protocolli leggeri (MQTT per la comunicazione interna ed HTTP REST per l’interfaccia utente), unitamente all’utilizzo di un server Flask asincrono, risulta pertanto efficace e coerente con i modelli di progettazione per sistemi IoT distribuiti \cite{Yoon2020, Amaral2018}.
...
Per testare la resilienza del Network Layer, è stata disattivata la rete Wi-Fi durante il normale funzionamento. Il modulo ESP8266 ha gestito autonomamente la riconnessione grazie a un
meccanismo di riconnessione automatica della libreria di rete, evitando
il blocco del sistema e consentendo il ripristino della sessione MQTT
e la risottoscrizione dei topic una volta ristabilita la connettività.
Nel frattempo, il controller locale Arduino ha continuato a operare normalmente, gestendo PIR, relè e chiusura temporizzata (FR4) secondo i requisiti di autonomia locale (NFR5). Il test conferma la capacità del sistema di funzionare in modalità degradata senza compromettere la sicurezza fisica.

\textbf{Ritardi o perdita temporanea del segnale GPS} \\
Sono state considerate condizioni di ricezione GPS degradata, caratterizzate da
aggiornamenti di posizione irregolari o temporaneamente assenti, tipiche di contesti
reali. Il sistema ha dimostrato tolleranza a tali condizioni grazie alla logica stateful
di aggiornamento del geofence, che evita attivazioni basate su singoli campioni
instabili o rumorosi.
...
Il comportamento del sistema è stato osservato in condizioni di segnale PIR instabile
o intermittente, al fine di verificare la robustezza della logica di automazione in uscita.
I meccanismi software adottati,basati su verifica dello stato del sistema e vincoli contestuali, hanno impedito l’attivazione di eventi indesiderati, evitando aperture
involontarie della porta. Il sistema ha continuato a operare in sicurezza, confermando
il soddisfacimento del requisito FR5a.
...
\subsection{Test NFR5 – Funzionamento Offline}

Il requisito non funzionale NFR5 stabilisce che il sistema debba garantire la piena continuità operativa anche in assenza di connettività Internet o della rete Wi-Fi domestica. Tale vincolo risponde a due esigenze fondamentali: (i) preservare la sicurezza fisica dell’utente assicurando la possibilità di chiudere la porta del garage anche in condizioni di fault del Network Layer, e (ii) prevenire situazioni di lock-out dovute alla dipendenza da servizi remoti. Nei sistemi IoT moderni, la capacità di operare in modalità \textit{offline degraded mode} rappresenta un criterio essenziale di robustezza e affidabilità \cite{Avizienis2004, Marwedel2021}.

\subsubsection*{Obiettivo del test}
Verificare che:
\begin{enumerate}
    \item la logica locale eseguita da Arduino continui a funzionare correttamente senza alcun supporto da parte del NodeMCU ESP8266;
    \item sia possibile aprire/chiudere la porta tramite il pulsante fisico (FR7) anche con rete disattivata;
    \item la chiusura automatica temporizzata (FR4) venga eseguita regolarmente;
    \item i sensori PIR e ultrasuoni continuino a funzionare regolarmente;
    \item il sistema non generi comportamenti incoerenti o pericolosi durante l’assenza di rete;
    \item il NodeMCU, una volta ripristinata la rete, ristabilisca automaticamente la connessione MQTT.
\end{enumerate}

\subsubsection*{Setup sperimentale}
Il test è stato eseguito in condizioni reali seguendo la procedura:
\begin{enumerate}
    \item disattivazione totale del Wi-Fi domestico;
    \item verifica dello stato del broker MQTT (irraggiungibile);
    \item avvio della porta in stato chiuso;
    \item attivazione manuale tramite pulsante fisico collegato al pin Arduino dedicato (FR7);
    \item simulazione di movimento tramite sensore PIR per osservare eventuali attivazioni involontarie;
    \item verifica della chiusura automatica allo scadere del timer locale (FR4);
    \item osservazione del comportamento del sistema durante la riattivazione del Wi-Fi.
\end{enumerate}

\subsubsection*{Risultati}
I risultati ottenuti sono riportati nella Tabella~\ref{tab:nfr5_offline}.

\begin{table}[h!]
\centering
\caption{Esito del test NFR5 – funzionamento in modalità offline}
\label{tab:nfr5_offline}
\begin{tabular}{|p{6cm}|p{6.5cm}|c|}
\hline
\textbf{Funzionalità testata} & \textbf{Descrizione comportamento osservato} & \textbf{Esito} \\ \hline

Pulsante locale (FR7) &
Il comando fisico attiva correttamente il relè anche in completa assenza di rete o broker MQTT. &
Superato \\ \hline

Chiusura automatica (FR4) &
Il timer locale su Arduino chiude la porta dopo 45 secondi, indipendentemente dal livello di connettività. &
Superato \\ \hline

Sensore PIR (FR5a) &
La rilevazione del movimento rimane operativa. Nessuna apertura involontaria registrata. &
Superato \\ \hline

Sensore distanza HC-SR04 (FR8) &
La rilevazione ostacoli funziona regolarmente; la porta non completa la chiusura se la distanza è $<15$ cm. &
Superato \\ \hline

Stato del sistema lato Flask &
Nessun aggiornamento dello stato porta viene ricevuto (atteso). Il sistema evita inconsistenze e non genera errori. &
Superato \\ \hline

Riconnessione NodeMCU &
All'attivazione del Wi-Fi, l’ESP8266 ristabilisce automaticamente la connessione MQTT con meccanismo di riconnessione automatica. &
Superato \\ \hline

Coerenza del sistema &
Nessun comportamento incoerente o pericoloso osservato; la logica locale garantisce la continuità operativa. &
Superato \\ \hline
...
Il test conferma pienamente il soddisfacimento del requisito NFR5. La logica locale, interamente eseguita da Arduino nei livelli \textit{Perception} e \textit{Actuation}, garantisce un funzionamento sicuro anche in assenza totale del \textit{Network Layer}. Questo comportamento è coerente con le linee guida per sistemi embedded real-time e IoT resilienti, che raccomandano l’adozione di controllori locali indipendenti dalla connettività esterna per garantire la \textit{safe fallback mode} \cite{Marwedel2021, Avizienis2004}.

L'ESP8266, pur risultando isolato dal broker MQTT durante la disconnessione, non influisce sul funzionamento operativo e ristabilisce autonomamente la connessione una volta disponibile la rete. Il sistema nel suo complesso si dimostra quindi \textbf{robusto}, \textbf{sicuro} e \textbf{coerente} con il modello architetturale progettato.


\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{images/latency_http_open_close.png}
    \caption{Latenza HTTP media del sistema di controllo per i comandi OPEN e CLOSE (solo request/response).}
    \label{fig:latency_http}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/latency_table_http_open_close.png}
    \caption{Statistiche della latenza HTTP (media e deviazione standard) per i comandi OPEN e CLOSE.}
    \label{fig:latency_http_table}
\end{figure}
...
Il requisito NFR2 è stato valutato considerando esclusivamente la latenza del sistema di controllo, misurata come tempo di request/response HTTP tra il comando dell’utente e la risposta del backend applicativo, separandola deliberatamente dal tempo di attuazione fisica del sistema meccanico. I test sperimentali mostrano una latenza media compresa tra 0.21 s e 0.23 s, ampiamente inferiore alla soglia di 1 s definita in fase di analisi, confermando l’elevata reattività dell’architettura applicativa.

Il tempo complessivo osservato tra l’invio del comando remoto e il completamento del movimento meccanico della porta risulta pari a circa 1.63--1.68 s. Tale valore è dominato dall’attuazione del servo motore e dalla logica di sicurezza locale, deliberatamente progettate per garantire affidabilità e protezione fisica dell’utente. Di conseguenza, questa misura non rientra nel perimetro del requisito NFR2, che è esplicitamente riferito alla latenza del sistema di controllo software, ma rappresenta una caratteristica progettuale del sottosistema di attuazione elettromeccanica.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{images/chart_open_close.png}
    \caption{Tempo medio complessivo di risposta del sistema (latenza HTTP + movimento del servo).}
    \label{fig:total_time_chart}
\end{figure}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.85\textwidth]{images/performance_table_full_open_close.png}
    \caption{Statistiche del tempo totale di risposta comprensive dell’attuazione meccanica.}
    \label{fig:total_time_table}
\end{figure}


Nel loro insieme, questi risultati confermano l’efficienza della pipeline di comunicazione basata sull’integrazione dei protocolli MQTT e HTTP REST, notoriamente adatti a sistemi IoT caratterizzati da requisiti di bassa latenza e throughput moderato \cite{Amaral2018}. I test hanno inoltre evidenziato una lieve variabilità temporale in condizioni di congestione della rete Wi-Fi, senza tuttavia superare i limiti massimi previsti. Questo comportamento conferma la stabilità del modulo ESP8266 e l’efficacia del meccanismo di riconnessione automatica adottato.

\textbf{Precisione e stabilità della geolocalizzazione} \\
Il modulo GPS ha mostrato un'accuratezza compatibile con le specifiche dei ricevitori GNSS
embedded (errore medio < 3--5 m) nella rilevazione della prossimità (FR5b), con un tasso di falsi positivi inferiore all'1\%, in linea con il requisito NFR3 e con quanto riportato dalla letteratura sui sensori GNSS per applicazioni embedded \cite{Zanella2014}. L'algoritmo di geofence, basato su variazioni di stato e non su campionamento continuo, ha confermato stabilità anche in presenza di jitter del segnale, grazie alla logica stateful basata su transizioni di stato stabili,
che evita attivazioni basate su singoli campioni rumorosi. La soglia operativa di 15 m ha mostrato prestazioni ottimali in scenari reali, con una rilevazione affidabile dell'ingresso e dell'uscita dal perimetro.
...
\begin{itemize}
    \item la latenza del sistema di controllo (HTTP request/response) rimane ampiamente inferiore al secondo;
...
\begin{itemize}
    ...
    \item \textbf{Reattivo}, con una latenza del sistema di controllo inferiore al secondo e un comportamento stabile anche in presenza di comandi rapidi in sequenza, grazie all’uso combinato dei protocolli MQTT e HTTP REST, progettati per garantire efficienza e leggerezza in contesti IoT \cite{Amaral2018}.
    ...
    \item \textbf{Sicuro}, grazie all'autenticazione HTTP con username e password, alla separazione rigorosa tra livello applicativo e livello fisico e alla cifratura fornita dal protocollo MTProto di Telegram \cite{Kuznetsov2018}, in conformità con i requisiti NFR6 e NFR7.
