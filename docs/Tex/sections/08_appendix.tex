...
% Nodo principale
\node[box, fill=gray!15, font=\bfseries, minimum width=65mm]
  {\texttt{SmartGarageDoor/}}

% ====================== HARDWARE ======================
child { node[box, fill=blue!5!white, minimum width=65mm]
          {\texttt{hardware/} \hfill \# Firmware Arduino / NodeMCU}
  child { node[box] {\texttt{controller\_arduino.ino}} }
  child { node[box] {\texttt{controller\_nodemcu.ino}} }
  child { node[box] {\texttt{FIRMWARE\_GUIDE.md}} }
  child { node[box] {\texttt{pinout\_table.csv}} }
}

% ====================== SOFTWARE ======================
child { node[box, fill=green!5!white, minimum width=65mm, yshift=-38mm]
          {\texttt{software/} \hfill \# Server Flask + Telegram Bot}
  child { node[box] {\texttt{app.py}} }
  child { node[box] {\texttt{telegram\_listener.py}} }
  child { node[box] {\texttt{timer.py}} }
  child { node[box] {\texttt{performance\_monitor.py}} }
}

% ====================== DOCS ======================
child { node[box, fill=yellow!10!white, minimum width=65mm, yshift=-76mm]
          {\texttt{docs/} \hfill \# Documentazione}
  child { node[box] {\texttt{Requisiti/}} }
  child { node[box] {\texttt{Tex/}} }
  child { node[box] {\texttt{Materiale\_in\_consegna/}} }
};
...
Principali file:
\begin{itemize}
    \item \texttt{controller\_arduino.ino} – gestione sensori e relè, timer di chiusura automatica;
    \item \texttt{controller\_nodemcu.ino} – connessione MQTT e bridge seriale con Arduino;
...
\begin{verbatim}
[NodeMCU] Connesso al broker mqtt://192.168.1.4
[Arduino] Il sensore PIR ha rilevato movimento → Apertura della porta
[Flask] L’utente lello ha inviato il comando /on
[Telegram] Notifica inviata: Porta aperta con successo
[GPS] Distanza < 15 m → Attivazione apertura automatica
[Arduino] Nessun movimento rilevato per 45 s → Chiusura della porta
\end{verbatim}
