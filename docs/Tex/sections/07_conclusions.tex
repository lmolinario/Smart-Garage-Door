...
\begin{tikzpicture}[
node distance=1.8cm,
block/.style={rectangle, draw, rounded corners, text width=3.2cm, align=center, minimum height=1cm}
]

\node (s) [block] {Scenario \\ e Assunzioni};
\node (r) [block, below of=s] {Analisi Requisiti \\ (FR / NFR)};
\node (d) [block, below of=r] {System Design \\ Architettura IoT};
\node (i) [block, below of=d] {Implementazione \\ Hardware \& Software};
\node (t) [block, below of=i] {Testing \\ e Validazione};
\node (o) [block, below of=t] {Risultati \\ Sperimentali};
\node (c) [block, below of=o] {Discussione \\ e Conclusioni};
...
\end{tikzpicture}
\caption{Workflow end-to-end del progetto Smart Garage Door secondo il modello SDLC adottato}
\label{fig:sdlc_overview}
\end{figure}

La Figura~\ref{fig:sdlc_overview} riassume il workflow end-to-end seguito nello sviluppo del sistema Smart Garage Door, evidenziando la continuità metodologica tra analisi, progettazione, implementazione e validazione sperimentale. Tale impostazione garantisce coerenza, tracciabilità e verificabilità delle scelte progettuali.
...
\begin{itemize}
    ...
    \item \textbf{Sicurezza e privacy}, supportate da autenticazione HTTP con username e password e dal modello crittografico MTProto di Telegram \cite{Kuznetsov2018};
