...
La comunicazione tra Arduino e NodeMCU avviene tramite la UART hardware (pin D10-D11), configurata a 9600 baud. Questa architettura consente uno scambio bidirezionale di segnali a bassa latenza e riflette le buone pratiche di interoperabilità nei sistemi embedded distribuiti \cite{Zanella2014}.
...
\begin{itemize}
    \item \texttt{/gps <lat> <lon>} --- Invia manualmente le coordinate GPS dell'utente per il calcolo del geofence (solo amministratori)
    \item \texttt{/setgarage <lat> <lon>} --- Aggiorna le coordinate del garage (solo amministratori)
...
L’analisi presentata in questa sezione fornisce una prima valutazione quantitativa delle prestazioni temporali del sistema.
Una discussione più approfondita dei risultati sperimentali, comprensiva di analisi comparativa, variabilità statistica e implicazioni progettuali, sarà sviluppata nel Capitolo~6, dedicato alla fase di test e validazione del sistema.
...
Dal punto di vista prestazionale, il sistema evidenzia tempi di risposta end-to-end prevedibili e compatibili con i requisiti applicativi, risultando adeguato a supportare sia comandi remoti sia automazioni locali in tempo quasi reale. La corretta sincronizzazione tra i moduli appartenenti ai tre livelli dell’architettura IoT — livello fisico, livello di rete e livello applicativo — consente una gestione coerente degli eventi e delle transizioni di stato, garantendo al contempo autonomia operativa, resilienza in caso di indisponibilità parziale della connettività e un adeguato livello di sicurezza dei comandi.
