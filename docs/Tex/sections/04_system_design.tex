...
\node[layerbox] (app) {\textbf{Application Layer}\\Flask Server, Digital Twin (DR/CVO), API REST, Telegram Interface};

\node[layerbox, below=5mm of app] (net) {\textbf{Network Layer}\\ESP8266 Gateway, MQTT Client, UART Bridge, HTTP Client};

\node[layerbox, below=5mm of net] (perc) {\textbf{Perception Layer}\\Arduino UNO, PIR Sensor, HC-SR04, Servo/Relè};

\node[comp, above=0mm of app.south west, anchor=south west, xshift=6mm] (api) {API REST};
\node[comp, right=5mm of api] (dt) {Digital Twin\\(DR/CVO)};
\node[comp, right=5mm of dt] (tg) {Telegram Bot};

\node[comp, above=0mm of net.south west, anchor=south west, xshift=6mm] (uart) {UART Bridge};
\node[comp, right=5mm of uart] (mqtt) {MQTT Client};
\node[comp, right=5mm of mqtt] (httpc) {HTTP Client};

\node[comp, above=0mm of perc.south west, anchor=south west, xshift=6mm] (pir) {PIR Sensor};
\node[comp, right=5mm of pir] (ultra) {HC-SR04};
\node[comp, right=5mm of ultra] (servo) {Servo/Relè};

\draw[->, thick] (perc.north) -- node[right]{Dati sensori} (net.south);
\draw[->, thick] (net.north) -- node[right]{Messaggi MQTT / HTTP} (app.south);
