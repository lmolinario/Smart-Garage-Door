...
\node[block, below left=15mm and 20mm of expired] (wait) {Continua attesa};
\node[block, below right=15mm and 20mm of expired] (check) {Verifica presenza ostacoli\\ (sensore distanza)};
...
\draw[arrow] (expired.west) -- ++(-45mm,0) |- (wait);
\draw[arrow] (wait) |- (expired);

\draw[arrow] (expired.east) -- ++(55mm,0) |- (check);
\draw[arrow] (check) -- (obstacle);

\draw[arrow] (obstacle.west) -- ++(-45mm,0) |- (abort);
\draw[arrow] (abort) |- (timer);

\draw[arrow] (obstacle.east) -- ++(55mm,0) |- (close);
...
\node[block, below right=15mm and 20mm of pircheck] (dist) {Lettura distanza\\ sensore ultrasuoni};
\node[decision, below=of dist] (distcheck) {Utente vicino alla porta?\\ (distanza < soglia)};

\node[block, below left=15mm and 20mm of distcheck] (stop2) {Nessuna apertura};
\node[startstop, below=of stop2] (end2) {Fine};

\node[block, below right=15mm and 20mm of distcheck] (open) {Arduino attiva relè\\ apertura porta};
...
\draw[arrow] (distcheck.west) -- ++(-45mm,0) |- (stop2);
\draw[arrow] (stop2) -- (end2);

\draw[arrow] (distcheck.east) -- ++(55mm,0) |- (open);
...
\node[block, below left=15mm and 20mm of nearhome] (noaction) {userNearHome = false\\ Nessuna automazione};
\node[startstop, below=of noaction] (end1) {Fine};

\node[block, below right=15mm and 20mm of nearhome] (setflag) {userNearHome = true};
...
\node[block, below right=15mm and 20mm of distcheck] (open) {Attiva relè\\ apertura porta};
...
\draw[arrow] (nearhome.east) -- ++(55mm,0) |- (setflag);
\draw[arrow] (setflag) -- (dist);
\draw[arrow] (dist) -- (distcheck);

\draw[arrow] (distcheck.west) -- ++(-45mm,0) |- (stop);
...
\node[block, below=of cmd] (extract) {Identificazione utente\\ (Telegram ID)};

\node[decision, below=of extract] (auth) {Utente autorizzato?};

\node[block, below left=15mm and 20mm of auth] (deny) {Comando rifiutato\\ + notifica accesso negato};
\node[block, below=of deny] (logdeny) {Log evento di sicurezza};
\node[startstop, below=of logdeny] (end1) {Fine};

\node[block, below right=15mm and 20mm of auth] (accept) {Autorizzazione concessa};
