...
descritto come la cooperazione di più cicli concorrenti, ognuno dei quali opera
su un diverso livello dell’architettura multilayer illustrata nella
Figura~\ref{fig:software_layers}:
\begin{itemize}
  \item \textbf{Perception Layer} (Arduino UNO): acquisizione dei sensori
        (PIR, HC-SR04), gestione del comando locale, attuazione del servo/relè,
        rilevazione degli ostacoli e generazione degli eventi di stato;
  \item \textbf{Network Layer} (NodeMCU ESP8266): instradamento dei comandi
        tra MQTT, HTTP e UART, gestione della connettività Wi-Fi e mantenimento
        del funzionamento locale in caso di assenza di Internet;
  \item \textbf{Application Layer} (Flask + timer + bot Telegram): gestione
        dello stato applicativo (DR), generazione del CVO, API REST, timer di
        health-check e interfaccia conversazionale con l’utente.
\end{itemize}

In questa sezione viene descritto il \textit{management algorithm} globale, che
integra le principali logiche implementate nei file
\texttt{controller\_arduino.ino}, \texttt{controller\_nodemcu.ino},
\texttt{app.py}, \texttt{timer.py} e
\texttt{telegram\_listener.py}.
L’obiettivo è fornire una vista unificata del flusso di controllo del sistema,
in grado di collegare in modo esplicito i requisiti funzionali FR1–FR9 con le
scelte architetturali e con il comportamento runtime.
...
Il flusso in Figura~\ref{fig:flowchart_sgd} evidenzia le principali fasi del funzionamento integrato del sistema:

\begin{enumerate}
\item \textbf{Acquisizione sensoriale (Arduino)}
Il microcontrollore esegue letture periodiche dei sensori PIR e HC-SR04, valutando la presenza dell’utente e la distanza da ostacoli o oggetti in prossimità della porta.
arduino
Copia codice
\item \textbf{Automazione FR5a–FR5b}
La porta si apre automaticamente solo quando:
\begin{itemize}
    \item il PIR rileva movimento (\textit{salita FR5a});
    \item la distanza è inferiore alla soglia;
    \item il NodeMCU segnala \texttt{userNearHome = true} tramite messaggio GPS (FR5b).
\end{itemize}
Tutte le condizioni devono essere soddisfatte simultaneamente per evitare aperture indesiderate.

\item \textbf{Comandi remoti da NodeMCU}
L’ESP8266 riceve richieste HTTP/MQTT provenienti dal server Flask e le inoltra ad Arduino tramite seriale, garantendo il funzionamento remoto e l’integrazione con il Digital Twin.

\item \textbf{Gestione API (Flask)}
Il server espone endpoint REST che permettono all’utente e ai servizi applicativi di:
\begin{itemize}
    \item aprire/chiudere la porta,
    \item interrogare lo stato corrente,
    \item ricevere notifiche e aggiornamenti in tempo reale.
\end{itemize}

\item \textbf{Chiusura automatica (FR4)}
Un timer interno ad Arduino garantisce la chiusura della porta dopo un intervallo configurabile, prevenendo aperture prolungate.

\item \textbf{Notifiche (MQTT + Telegram)}
Eventi significativi, come l’apertura della porta o il cambio dello stato GPS, vengono inoltrati tramite MQTT e Telegram, offrendo tracciabilità e trasparenza all’utente finale.
\end{enumerate}
...
\item \textbf{Modulo GPS} – utilizzato per implementare l’automazione basata sulla prossimità del veicolo (FR5b).  
...
\item \textbf{requests} – per le comunicazioni HTTP e componenti esterni.
\end{itemize}
...
\item \textbf{Token di sessione e autenticazione HTTP}  
Le richieste verso Flask sono validate tramite autenticazione HTTP con username e password, la quale genera un token di sessione che viene conservato lato server, impedendo l’invio di comandi da fonti non autorizzate.
...
I log generati dal sistema vengono anonimizzati (rimozione di ID personali), in conformità al requisito NFR7 sulla privacy.
