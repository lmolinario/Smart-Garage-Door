...
Le principali soluzioni commerciali attualmente disponibili per l’automazione delle porte da garage possono essere ricondotte a due macro-categorie:

\begin{itemize}
    \item \textbf{Sistemi proprietari}, sviluppati da aziende specializzate e basati su infrastrutture chiuse. Tali soluzioni fanno tipicamente affidamento su server cloud centralizzati per la gestione dei dispositivi e delle comunicazioni, utilizzando applicazioni dedicate e protocolli non interoperabili (ad es. Chamberlain, Tailwind, Nexx).

    \item \textbf{Sistemi aperti o compatibili}, progettati per integrarsi con ecosistemi domotici esistenti come Google Home, Amazon Alexa o Home Assistant. Queste soluzioni adottano protocolli standard e interoperabili, quali MQTT o Zigbee, favorendo l’estensibilità e l’integrazione con dispositivi di terze parti.
\end{itemize}

L’analisi condotta in questo lavoro si concentra esclusivamente sui \textbf{sistemi proprietari}, in quanto rappresentano le soluzioni maggiormente comparabili e direttamente concorrenti rispetto al sistema sviluppato nel presente progetto. Tale scelta consente di evidenziare in modo più chiaro i limiti delle architetture chiuse e di mettere in luce i vantaggi di un approccio progettuale aperto, modulare e orientato all’interoperabilità.
...
\midrule
Architettura & Cloud proprietario & Cloud + BLE proprietario & Cloud Wi-Fi & Locale-first + cloud opzionale \\
...
Automazioni uscita & No & No & No & GPS + PIR + sensori locali \\
...
La soluzione proposta si distingue per l’adozione di un’architettura locale-first,
che garantisce continuità di servizio, maggiore controllo dei dati e indipendenza
da infrastrutture cloud proprietarie.
...
\begin{itemize}
    \item \textbf{Architettura ibrida locale–remota}, in cui la logica di controllo primaria risiede nei microcontrollori (Arduino Uno ed ESP8266), garantendo operatività anche in assenza di connettività Internet. Tale proprietà, direttamente collegata al requisito \textbf{FR5 (sia versione a che versione b)} che approfondiremo nel prossimo Capitolo, consente al sistema di evitare dipendenze critiche da server esterni e di mantenere continuità di servizio in scenari degradati.
    ...
    \item \textbf{Automazione di prossimità basata su GPS}, implementata tramite il modulo \texttt{controller\_nodemcu.ino} del NodeMCU, preferita al BLE per la maggiore stabilità e accuratezza in ambienti outdoor. Questa soluzione permette di ridurre i falsi positivi in fase di rilevamento ingresso-uscita, in accordo con i principi di efficienza delle risorse e affidabilità funzionale.
...
Assenza di gestione utenti avanzata; scalabilità limitata a una singola autorimessa senza ulteriore hardware; dipendenza dalla copertura Wi-Fi domestica; sicurezza basata su meccanismi minimi. \\ \hline
